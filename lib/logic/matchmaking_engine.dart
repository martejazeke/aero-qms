...
class MatchmakingEngine {
  ...
  /// Respects hard preferred-partner constraint before scoring.
  MatchResult findBestMatch(
    ...
    // For doubles: enforce hard partner constraint
    if (playersNeeded == 4) {
      return _selectDoubles(scored, waitingRoom);
    } else {
  ...
  MatchResult _selectDoubles(
      List<PlayerScore> scored, List<Player> waitingRoom) {
    final selected = <Player>[];
    final usedIds  = <String>{};

    // Pass 1: find partner pairs where BOTH are waiting.
    // Pick the pair whose highest-scored member ranks best.
    ...
    for (final ps in scored) {
      ...
      if (partnerInRoom.isNotEmpty) {
        ...
        if (pairs.length == 2) break; // we have two pairs, done
      }
    }

    // Add pairs first (up to 2 pairs = 4 players for doubles)
    for (final pair in pairs) {
      selected.addAll(pair);
      if (selected.length >= 4) break;
    }

    // Pass 2: fill remaining slots from ranked non-partner players
    for (final ps in scored) {
      if (selected.length >= 4) break;
  ...
  List<Player> _selectFair(List<PlayerScore> scored, int needed) {
    final fresh    = scored.where(
        (ps) => !_lastMatchIds.contains(ps.player.id)).toList();
    final repeated = scored.where(
        (ps) =>  _lastMatchIds.contains(ps.player.id)).toList();
  ...
  PlayerScore _scorePlayer(Player player, List<Player> room) {
    ...
    final waitScore  = maxWait > 0
        ? (playerWait / maxWait).clamp(0.0, 1.0) : 0.0;

    final maxGames  = _maxGames(room);
    final gamesScore = maxGames > 0
        ? (1.0 - player.gamesPlayed / maxGames) : 1.0;
